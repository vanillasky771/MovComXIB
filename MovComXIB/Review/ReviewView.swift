...
extension ReviewView {
    ...
    func makeRequest(numHits: Int, page: Int) -> URLRequest {
        let url = URL(string:Constants.host+"movie/\(numHits)/reviews?api_key=765f90b887428e11c9c3f8439cd80f8b&language=en-US&page=\(page)")!
        return URLRequest(url: url)
    }
    
    func fetchData(handler: @escaping ((FetchResult) -> Void)){
        let request = makeRequest(numHits: movIds, page: currentPages)
